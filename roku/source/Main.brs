...
Sub Init()
  ...
  m.horses = CreateObject("roBitmap", "pkg:/images/horses.png")
  ...
  m.sprites = CreateObject("roArray", 8, True)
  for i = 0 to 7
    spawn = m.data.spawns[i]
    region = CreateObject("roRegion", m.horses, i*(2*m.R), 0, 2*m.R, 2*m.R)
    m.sprites[i] = m.compositor.NewSprite(spawn.x, spawn.y, region)
    m.sprites[i].SetData({ x: spawn.x, y: spawn.y, theta: Rnd(0) * 2 * m.PI })
  end for
